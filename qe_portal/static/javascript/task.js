function task_delete(id, url){
    $.ajax({
       type: 'POST',
       // Here, we define the used method to send data to the Django views.
       // Other values are possible as POST, GET, and other HTTP request methods.
        url: url,
...
function task_delete_confirm(response) {
    ...
    if (task_id>0) {
       ...
       alert('Error, task id less than 0');
} }